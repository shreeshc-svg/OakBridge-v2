...
import { GraduationCap, BookOpen, Award, MonitorPlay } from "lucide-react";
...
const ICONS = { MonitorPlay, GraduationCap, BookOpen, Award };
...
export default function Academy() {
    ...
    useEffect(() => {
        ...
        fetchCollection("page_ac_features").then(setFeatData).catch(() => {});
        fetchCollection("page_ac_stats").then(setStatsData).catch(() => {});
    }, []);

    const resolvedFeatures = resolveCollection(featData, DEFAULT_FEATURES).map((f) => ({
        ...f,
        icon: ICONS[f.icon] || MonitorPlay,
    }));
